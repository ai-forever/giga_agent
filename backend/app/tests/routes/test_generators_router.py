...
class GeneratorsRouterTests(unittest.TestCase):
    ...
    def _response_payload(self, generator_obj) -> dict:
        return {
            ...
            "llm_provider_id": (
                str(generator_obj.llm_provider_id)
                if generator_obj.llm_provider_id is not None
                else None
            ),
    ...
    def test_create_success_for_supported_type_with_llm_provider(self):
        created = self._generator_obj(generator_type="openai", llm_provider_id=uuid.uuid4())

        with patch(
            "giga_agent.routes.generators.image._resolve_runtime_cls",
            return_value=types.SimpleNamespace(supported_llm_provider_types=lambda: ["openai"]),
        ), patch(
            "giga_agent.routes.generators.image._validate_settings",
            AsyncMock(return_value={"model": "dall-e-3"}),
        ), patch(
            "giga_agent.routes.generators.image._validate_llm_provider_link",
            AsyncMock(return_value=created.llm_provider_id),
        ), patch(
            ...
            response = self.client.post(
                ...
                json={
                    ...
                    "llm_provider_id": str(created.llm_provider_id),
                    "is_active": True,
                },
            )
    ...
    def test_get_generator_types_meta(self):
        runtime_map = {
            "openai": types.SimpleNamespace(
                supported_llm_provider_types=lambda: ["openai"]
            ),
            "fusion_brain": types.SimpleNamespace(
                supported_llm_provider_types=lambda: []
            ),
        }
        ...
        self.assertEqual(
            ...
            [
                {
                    "type": "openai",
                    "supported_llm_provider_types": ["openai"],
    ...
    def test_create_success_for_unsupported_type_without_llm_provider(self):
        created = self._generator_obj(generator_type="fusion_brain", llm_provider_id=None)

        with patch(
            "giga_agent.routes.generators.image._resolve_runtime_cls",
            return_value=types.SimpleNamespace(supported_llm_provider_types=lambda: []),
        ), patch(
            "giga_agent.routes.generators.image._validate_settings",
    ...
    def test_patch_without_type_updates_allowed_fields(self):
        ...
        with patch(
            ...
            "giga_agent.routes.generators.image._validate_llm_provider_link",
            AsyncMock(return_value=existing.llm_provider_id),
        ), patch(
    ...
    def test_deactivate_current_auto_clears_current(self):
        generator_id = uuid.uuid4()
        existing = self._generator_obj(generator_id=generator_id, is_active=True)
        updated = self._generator_obj(generator_id=generator_id, is_active=False)
