...
class GrokImagineToolTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_gen_image_grok_reads_input_images_and_uploads_result(self):
        owner_id = uuid.uuid4()
        runtime = self._runtime(owner_id)
        user = types.SimpleNamespace(id=owner_id, image_generator_id=uuid.uuid4())
        generator = GrokImagineImageGen(api_key="test-key")
        ...
        with patch(
            "giga_agent.generators.image.grok.tool.get_session_factory",
            AsyncMock(return_value=lambda: _session_context()),
        ), patch(
            "giga_agent.generators.image.grok.tool.UserRepository.get_cached_or_db",
            AsyncMock(return_value=user),
        ), patch(
            "giga_agent.generators.image.grok.tool.ImageGeneratorManager.resolve_by_id",
            AsyncMock(return_value=generator),
        ), patch(
            "giga_agent.generators.image.grok.tool.SandboxManager.read_file_by_path_for_user",
            AsyncMock(
                return_value=(
                    object(),
            ...
            "giga_agent.generators.image.grok.tool.SandboxManager.upload_files_for_user",
            AsyncMock(return_value=types.SimpleNamespace(files=[uploaded_file])),
        ) as mocked_upload, patch.object(
            GrokImagineImageGen,
            "generate_image",
            new=AsyncMock(return_value=base64.b64encode(b"generated-image").decode("ascii")),
        ) as mocked_generate:
    ...
    async def test_gen_image_grok_rejects_non_image_inputs(self):
        owner_id = uuid.uuid4()
        runtime = self._runtime(owner_id)
        user = types.SimpleNamespace(id=owner_id, image_generator_id=uuid.uuid4())
        generator = GrokImagineImageGen(api_key="test-key")
