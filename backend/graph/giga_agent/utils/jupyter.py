    ...
    async def upload_run_files(
        ...
        *,
        ...
        url = (
            ...
            f"{endpoint if endpoint.startswith('/') else '/' + endpoint}"
        )
        ...
        for file in files:
            filename = file.path
            # Если путь не указан, filename может быть None — aiohttp примет его,
            # но серверу может понадобиться default
...
tool_client.set_state_data({thread_id!r}, {checkpoint_id!r})"""
...
# fig_price_hist = px.histogram(df, x='price', nbins=50, title='Распределение цен на квартиры') # noqa: E501
# fig_price_hist.show()
#
# # Диаграмма рассеяния площади vs цена
# fig_scatter_area_price = px.scatter(df, x='area', y='price', color='rooms', title='Площадь против цены квартир') # noqa: E501
