...
RULES: tuple[ToolsetRule, ...] = (
    ...
    ToolsetRule(
        ...
        keywords=("календар", "событи", "встреч", "напомни", "запланир", "расписан",
                  "calendar", "event", "митинг", "созвон"),
        tools=("calendar_",),
    ),
    ToolsetRule(
        name="mock_tracker",
    ...
    # MCP Битрикс24 (подключается пользователем через модалку «Серверы MCP»).
    # Имена тулзов дефисные (bitrix-search …) — префикс ловится _resolve.
    ToolsetRule(
        name="bitrix",
        keywords=("битрикс", "bitrix", "b24", "crm", "црм", "сделк", "лид",
                  "lead", "deal", "воронк", "смарт-процесс", "smart process",
                  "контакт", "компани"),
        tools=("bitrix-",),
    ),
    ToolsetRule(
...
TOOL_KEYWORDS: dict[str, tuple[str, ...]] = {
    ...
    "disk_unpublish": ("сними публикац", "закрой доступ", "убери ссылк",
                       "перестань делиться", "unpublish", "сделай приватн"),
    ...
    "mail_send": ("отправ", "напиши письмо", "напиши на", "ответь", "ответить",
                  "написать письмо", "mail_send", "send"),
    ...
    # --- Яндекс.Календарь ---
    "calendar_list_events": ("календар", "ближайшие событи", "что запланир",
                             "расписан", "какие встреч", "мои событи", "повестк"),
    "calendar_month": ("месяц", "на месяц", "покажи июн", "покажи июл", "грид",
                       "календар на", "весь месяц", "сетк"),
    "calendar_create_event": ("создай событи", "добавь в календар", "запланир",
                              "поставь встреч", "назначь", "напомни мне"),
    "calendar_delete_event": ("удали событи", "отмени встреч", "убери из календар"),
}
