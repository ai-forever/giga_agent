...
def _validate_relative(relative_path: str) -> Path:
    ...
    if not clean or any(part in {"", ".", ".."} for part in rel.parts):
        raise HTTPException(status_code=400, detail=f"Invalid relative path: {relative_path!r}")
    return rel
...
class SkillsManager:
    ...
    def read_file(self, owner_id: str | None, skill_name: str, relative_path: str) -> str:
        ...
        if not target.is_file():
            raise HTTPException(status_code=404, detail=f"Skill file not found: {relative_path}")
...
router = APIRouter(prefix="/v1/skills", tags=["skills"], dependencies=[Depends(require_token)])
