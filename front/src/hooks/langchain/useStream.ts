...
import {
  ...
  GetUpdateType,
  RunCallbackMeta,
} from "@langchain/langgraph-sdk/dist/react/types";
import { findLast, unique } from "@/hooks/langchain/utils.ts";
import { StreamError } from "@/hooks/langchain/error.ts";
import { getBranchContext } from "@/hooks/langchain/branching.ts";
import { MessageTupleManager } from "@/hooks/langchain/messages.ts";
import { EventStreamEvent, StreamManager } from "@/hooks/langchain/manager.ts";
import { useControllableThreadId } from "@/hooks/langchain/thread.ts";
...
>(options: UseStreamOptions<StateType, Bag>): UseStream<StateType, Bag> {
  ...
  const emptyState = useMemo(() => ({} as StateType), []);
  const historyValues = useMemo(
    () =>
      branchContext.threadHead?.values ?? options.initialValues ?? emptyState,
    [branchContext.threadHead?.values, options.initialValues, emptyState],
  );
