...
- работать с разными моделями, [доступными в LangChain](https://python.langchain.com/docs/integrations/chat/#all-chat-models): GigaChat, ChatGPT, Anthropic, DeepSeek и другими
- исполнять код в чате с помощью REPL-среды, подобной [блокнотам Jupyter](https://jupyter.org/)
- обмениваться данными со сторонними сервисами: VK, 2GIS, Яндекс.Диск, Яндекс.Почта, Яндекс.Календарь и другими
- использовать инструменты для анализа данных, генерации изображений, создания презентаций и лендингов
- проводить глубокое исследование темы (Deep Research) с планом, поиском и отчётом с цитатами
- генерировать изображения с помощью разных провайдеров: GigaChat, FusionBrain, OpenAI, Grok, Nano Banana
- планировать отложенные и периодические фоновые задачи
- группировать чаты в проекты с общими инструкциями и собственной базой знаний
- общаться с агентом из Telegram через собственного бота
...
- **Projects** — группировка чатов в проекты с общими инструкциями и своей базой знаний (RAG)
- **Message Branching** — ветвление диалогов для экспериментов
- **File Attachments** — поддержка изображений, документов, аудио, видео (drag&drop и вставка из буфера)
- **Voice Input** — голосовой ввод через SaluteSpeech (press-to-talk)
- **Chat Export** — экспорт диалога в PDF, DOCX и Markdown
- **Prompt Suggestions** — стартовые рекомендации и контекстные follow-up подсказки
- **Clarifying Questions** — агент может задавать уточняющие вопросы перед выполнением
- **Tool Visualization** — визуализация выполнения инструментов в UI
- **Generative UI Widgets** — интерактивные виджеты в чате (почта, календарь, файлы, доски задач)
- **Onboarding** — интерактивный гид по возможностям и настройкам
- **Auto-approve Mode** — автоматическое подтверждение использования инструментов
- **Stop Button** — прерывание выполнения агента

### 🔧 Встроенные модули

GigaAgent включает набор готовых модулей для разных задач. Модули можно включать и
выключать в настройках; часть из них — служебные и работают всегда.

| Модуль                | ID                 | Описание                                                                     |
| --------------------- | ------------------ | ---------------------------------------------------------------------------- |
| **Песочница (REPL)**  | `repl`             | Выполнение Python и shell-команд в изолированной среде                        |
| **Файловые операции** | `io`               | Чтение, запись и редактирование файлов                                       |
| **Генерация изображений** | `image`        | Создание изображений по текстовому описанию                                  |
| **Анализ изображений**| `analyze_images`   | Распознавание и описание содержимого изображений                             |
| **Поиск**             | `search`           | Поиск информации в интернете                                                 |
| **Веб-скрапер**       | `scraper`          | Извлечение данных с веб-страниц по URL                                       |
| **Документы (RAG)**   | `rag`              | Поиск и извлечение информации из загруженных документов                      |
| **Deep Research**     | `deep_research`    | Глубокое исследование: план → поиск → чтение → синтез отчёта                 |
| **Отложенные задачи** | `scheduler`        | Планирование отложенных и периодических фоновых задач                        |
| **Память**            | `memory`           | Долговременная память агента                                                 |
| **Навыки (Skills)**   | `skills`           | Использование пользовательских навыков (skill-манифестов)                    |
| **MCP**               | `mcp`              | Подключение внешних MCP-серверов и вызов их инструментов                     |
| **MCP (фронтенд)**    | `frontend_mcp`     | Вызов MCP-тулов на стороне фронтенда пользователя                            |
| **Погода**            | `weather`          | Получение прогноза погоды через OpenWeatherMap                               |
| **VK**                | `vk`               | Работа с API ВКонтакте                                                       |
| **Яндекс.Диск**       | `yandex_disk`      | Работа с файлами и папками на Яндекс.Диске                                   |
| **Яндекс.Почта**      | `yandex_mail`      | Чтение и отправка писем в Яндекс.Почте                                       |
| **Яндекс.Календарь**  | `yandex_calendar`  | Просмотр и создание событий в Яндекс.Календаре                               |
| **Креатив-агенты**    | `subagents_legacy` | Lean Canvas, генерация мемов, подкастов, лендинги, презентации, исследование городов |
| **Проекты**           | `projects`         | Инструкции и база знаний уровня проекта                                      |
| **Аутентификация**    | `auth`             | Аутентификация, управление пользователями и группами (служебный)            |
...
- Яндекс.Диск, Яндекс.Почта, Яндекс.Календарь — сервисы Яндекса (OAuth)
- OpenWeatherMap — прогноз погоды
- SaluteSpeech — распознавание речи для голосового ввода

**Channels:**

- Telegram — общение с агентом через собственного бота

**Vector Store:**

- Qdrant — для RAG и памяти
...
- **[LangGraph 1.x](https://github.com/langchain-ai/langgraph)** — state machine для AI-агентов от LangChain
